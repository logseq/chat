...
final class _SearchPresentationState extends State<_SearchPresentation> {
  ...
  void initState() {
    ...
    _controller.text = widget.context.property('query')! as String;
  }
    ...
    return PopScope<Object?>(
      ...
      child: Column(
        children: [
              ...
              child: Semantics(
                ...
                child: SearchBar(
                  ...
                  leading: IconButton(
                    ...
                    onPressed: () => extension.emit(
                      name: depth > 0 ? 'back' : 'dismiss',
                      values: depth > 0 ? const {'count': 1} : const {},
                    ),
                  ),
          ...
          Expanded(child: extension.content),
        ],
      ),
    );
  }
        ...
        child: DefaultTextStyle.merge(
          ...
          child: isAsset
              ...
              : _ProjectedMarkup(
                  ...
                  youtubePlayback: youtubePlayback,
                  onOpenNode: (nodeUuid) => context.emit(
                    name: 'open-node',
                    values: {'uuid': nodeUuid},
                  ),
                ),
        ),
      ),
...
final class _OutlinerEditorState extends State<_OutlinerEditor> {
  ...
  void _handleTextChanged(String title) {
    final newline = title.indexOf('\n');
    if (newline >= 0) {
      _textChangeTimer?.cancel();
      final submitted = title.replaceAll('\n', '');
      final caret = (_caret - 1).clamp(0, submitted.length);
      _applyingModel = true;
      try {
