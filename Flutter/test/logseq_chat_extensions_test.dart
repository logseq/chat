...
void main() {
  Future<LUIFlutterBackend> mountEditor(
    WidgetTester tester,
    List<LUIEvent> events, {
    String title = 'Before',
  }) async {
    ...
    backend.applyJson(
      jsonEncode({
        'generation': 1,
        'ops': [
          ...(_editorPatch['ops']! as List<Map<String, Object>>),
          {
  ...
  Iterable<LUIExtensionComponentEvent> edits(
    List<LUIEvent> events,
    String name,
  ) => events.whereType<LUIExtensionComponentEvent>().where(
    (event) => event.name == name,
  );

  testWidgets(
    'typing in an existing multiline block preserves its line breaks',
    (tester) async {
      final events = <LUIEvent>[];
      await mountEditor(tester, events, title: 'first\nsecond');
      await tester.enterText(find.byType(TextField), 'first\nsecond!');
      await tester.pump(const Duration(milliseconds: 100));
      expect(edits(events, 'return'), isEmpty);
      expect(
        edits(events, 'text-change').single.values['title'],
        'first\nsecond!',
      );
    },
  );

  testWidgets('pasting multiline content does not create a block', (
    tester,
  ) async {
    final events = <LUIEvent>[];
    await mountEditor(tester, events);
    await tester.enterText(find.byType(TextField), 'first\nsecond');
    await tester.pump(const Duration(milliseconds: 100));
    expect(edits(events, 'return'), isEmpty);
    expect(
  ...
  testWidgets('Enter removes only the newly inserted line break', (
    tester,
  ) async {
    final events = <LUIEvent>[];
    await mountEditor(tester, events, title: 'first\nsecond');
    await tester.enterText(find.byType(TextField), 'first\nsecond\n');
    await tester.pump(const Duration(milliseconds: 100));
    expect(edits(events, 'return').single.values, {
      'title': 'first\nsecond',
      'caret-utf16-offset': 12,
    });
    expect(edits(events, 'text-change'), isEmpty);
  });

  testWidgets('Shift Enter inserts a newline without submitting', (
    tester,
  ) async {
    final events = <LUIEvent>[];
    await mountEditor(tester, events);
    ...
    await tester.pump(const Duration(milliseconds: 100));
    expect(edits(events, 'return'), isEmpty);
    expect(edits(events, 'text-change').single.values['title'], 'Before\n');
  });

  testWidgets('unrelated model updates cannot overwrite pending typing', (
    tester,
  ) async {
    final events = <LUIEvent>[];
    final backend = await mountEditor(tester, events);
    ...
    await tester.pump(const Duration(milliseconds: 100));
    expect(edits(events, 'text-change').last.values['title'], 'Before!');
  });

  testWidgets('model caret updates apply when block text is unchanged', (
    tester,
  ) async {
    final events = <LUIEvent>[];
    final backend = await mountEditor(tester, events);
  ...
  testWidgets('IME composition is published only after committing', (
    tester,
  ) async {
    final events = <LUIEvent>[];
    await mountEditor(tester, events);
    ...
    await tester.pump(const Duration(milliseconds: 100));
    expect(edits(events, 'text-change').single.values['title'], 'Before中文');
  });

  testWidgets(
    'search offers a clear action without dismissing or losing input focus',
    (tester) async {
      final events = <LUIEvent>[];
      final backend = LUIFlutterBackend(
        extensionRegistry: logseqChatExtensionRegistry(),
      ...
      expect(edits(events, 'query-changed').last.values['query'], '');
      expect(edits(events, 'dismiss'), isEmpty);
    },
  );

  for (final title in ['Before', 'first\nsecond']) {
    testWidgets('read and edit layout retain the same height for $title', (
      tester,
    ) async {
      final readBackend = LUIFlutterBackend(
        extensionRegistry: logseqChatExtensionRegistry(),
      );
      addTearDown(readBackend.dispose);
      readBackend.applyJson(
        jsonEncode({
          'generation': 1,
          'ops': [
      ...
      await tester.pumpWidget(
        MaterialApp(
          theme: LogseqChatTheme.light(),
          home: Scaffold(body: readBackend.widget(node: 1)),
        ),
      );
      final height = tester
          .getSize(find.byKey(const ValueKey('outliner-block-content-block-a')))
          .height;
      await mountEditor(tester, <LUIEvent>[], title: title);
      expect(
  ...
  testWidgets(
    'search ignores unrelated patches while typing and animates dismissal',
    (tester) async {
      final events = <LUIEvent>[];
      final backend = LUIFlutterBackend(
        extensionRegistry: logseqChatExtensionRegistry(),
      ...
      await tester.enterText(find.byType(SearchBar), 'local query');
      backend.applyJson(
        jsonEncode({
          'generation': 2,
          'ops': [
      ...
      expect(edits(events, 'dismiss'), isEmpty);
      await tester.pump(const Duration(milliseconds: 200));
      expect(edits(events, 'dismiss'), hasLength(1));
    },
  );

  testWidgets(
    'delayed editing acknowledgements preserve newer text and caret',
    (tester) async {
      final events = <LUIEvent>[];
      final backend = await mountEditor(tester, events);
      await tester.enterText(find.byType(TextField), 'Before one');
      await tester.pump(const Duration(milliseconds: 100));
      await tester.enterText(find.byType(TextField), 'Before one two');
      await tester.pump(const Duration(milliseconds: 100));
      ...
      await tester.pump();
      expect(controller.text, 'Before one two');
