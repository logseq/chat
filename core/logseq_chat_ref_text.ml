(* Convert block titles between the stored uuid-reference form
   ("[[uuid]]", "#[[uuid]]") and the text form shown in the editor
   ("[[Page name]]", "#Tag"), mirroring Logseq's id-ref->title-ref and
   title-ref->id-ref conversions. *)
...
(* A tag label that can be written as a bare "#label" hashtag. *)
...
(* Stored form -> editor text. [tag_title] and [ref_title] resolve a uuid to
   the display label; returning [None] keeps the uuid form (unknown entity or
   an ambiguous name that must stay uuid-addressed). *)
...
(* Editor text -> stored form. [resolve_ref] and [resolve_tag] resolve a name
   to the uuid of a unique matching entity; unresolved names are kept as
   plain text. *)
let to_ids ~resolve_ref ~resolve_tag title =
  ...
  let rec loop index =
    ...
    then (
      let word_end =
        let rec find at = if at < length && not (tag_terminator title.[at]) then find (at + 1) else at in
        find (index + 1)
      in
