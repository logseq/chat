...
let block db eid =
  match uuid_for_eid db eid, string_value (value db eid "block/title") with
  | Some uuid, Some title when Option.is_none (value db eid "block/name") ->
...
let blocks db =
  let entity_ids =
    ...
    |> Seq.fold_left (fun ids datom -> Int_set.add datom.e ids) Int_set.empty
  in
  entity_ids
  |> Int_set.to_seq
