...
let decode_row = function
  | Value.Array [ addr; Value.String content; addresses ] ->
    bind (int_of_value addr) (fun addr ->
      bind (optional_string addresses) (fun addresses ->
  ...
  let decode value =
    ...
    | Value.Array values ->
      let rec loop rows = function
        | [] -> Ok (List.rev rows)
        | value :: rest ->
          bind (decode_row value) (fun row -> loop (row :: rows) rest)
      in
      loop [] values
    | _ -> Error "snapshot frame payload must be a Transit row array"
  in
  try decode (Codec.of_string payload) with
...
let finish_import state =
  if state.baseline_t < 0
  then Error "snapshot baseline cursor must be non-negative"
  else if state.expected_rows < 0
